 function [ret,combined]=MinutiaeCN(thinned,mask,orient_img,orient_img_m,cimg)
...
% Filtering False Minutiae ------------------------------------------------
    ...
    if core_val < 1
      minutiae(minu_count+1, :) = [core_x, core_y, 5, start_t]; 
      minu_count = minu_count + 1;
    end
    if dt1 < 1
       minutiae(minu_count+1, :) = [delta1_x, delta1_y, 7, 0];
       minu_count = minu_count + 1;
    end
    if dt2 < 1       
       minutiae(minu_count+1, :) = [delta2_x, delta2_y, 7, 0];
       minu_count = minu_count + 1;
    end
    if dt3 < 1       
       minutiae(minu_count+1, :) = [delta3_x, delta3_y, 7, 0];
       minu_count = minu_count + 1;
    end
